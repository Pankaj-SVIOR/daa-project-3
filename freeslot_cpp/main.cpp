// =====================================================
// main.cpp  —  Teacher Free Slot Finder (C++ Server)
// =====================================================
// All routes in one file. Clean and simple.
// Auth: login/logout/session via cookie
// Pages: dashboard, today-classes, book-room, etc.
// =====================================================

#include "utils/server.h"    // HTTP server (WinSock2)
#include "utils/database.h"  // SQLite helpers
#include "models/dsa.h"      // Stack, Queue, Linked List, Hash Table
#include "models/session.h"  // Session management (Hash Map)
...
// =====================================================
// SLOT MAPS — Hash Map: slot number <-> time string
// =====================================================
std::map<int, std::string> slotToTime;
std::map<std::string, int> timeToSlot;

void initSlotMaps() {
    // Using Hash Map for fast slot<->time lookup
...
// =====================================================
// GLOBAL DATA STRUCTURES (DAA)
// =====================================================
BookingStack undoStack;   // Stack — undo booking (LIFO)
BookingQueue bookQueue;   // Queue — booking queue (FIFO)
std::map<int, BookingHistory*> roomHistories;  // Hash Map — room history
...
std::string readFile(const std::string& name) {
    ...
    if (!f.is_open()) return "<h1 style='color:red;font-family:sans-serif'>Error: views/" + name + " not found. Run from freeslot_cpp/ folder.</h1>";
...
// Auth check helper — returns teacher_id or -1
...
int main() {
    ...
    server.Get("/logout", [](const Request& req, Response& res) {
        std::string token = getCookie(req, COOKIE_NAME);
        if (!token.empty()) destroySession(token);  // Using Hash Map — delete session
        res.clear_cookie(COOKIE_NAME);
        res.redirect("/login");
    });

    // ==================================================
    // PROTECTED PAGE ROUTES (login required)
    // ==================================================
    ...
    // POST /api/login  — check name+password, create session
    server.Post("/api/login", [](const Request& req, Response& res) {
        ...
        // Using Hash Map (DB) — find teacher by name
        sqlite3* db = openDB();
        ...
        // Using Hash Map — createSession stores token->teacher_id
    ...
    // POST /api/reset-password  — update password (no old pass needed)
    ...
    // GET /api/me  — returns logged-in teacher's info (used by all dashboard pages)
    server.Get("/api/me", [](const Request& req, Response& res) {
        std::string token = getCookie(req, COOKIE_NAME);
        int tid = getSessionTeacherId(token);  // Using Hash Map
    ...
    // GET /free_rooms_today
    // DAA: Using Set (O(1) busy check), Sorting (slot order 1..10)
    server.Get("/free_rooms_today", [](const Request&, Response& res) {
        std::string today = todayDay();
        sqlite3* db = openDB(); if (!db) { res.set_content(jerr("DB error"),"application/json"); return; }
        std::map<int,std::string> allRooms;  // Using Hash Map
        sqlite3_stmt* st = NULL;
        ...
        sqlite3_finalize(st);
        std::map<int,std::set<int> > busyBySlot;  // Using Set
        ...
        for(int s=1;s<=10;s++) {  // Using Sorting — iterate in slot order
            ...
            for(rit=allRooms.begin();rit!=allRooms.end();++rit) {
                bool busy = busyBySlot.count(s)&&busyBySlot[s].count(rit->first);  // Using Set
    ...
    // GET /teacher_classes_today/<id>
    // DAA: Using Sorting (ORDER BY slot)
    server.Get("/teacher_classes_today/([0-9]+)", [](const Request& req, Response& res) {
        ...
        while(sqlite3_step(st)==SQLITE_ROW) {
            ...
            std::string ts=slotToTime.count(sn)?slotToTime[sn]:std::to_string(sn);  // Using Hash Map
    ...
    // GET /api/weekly_schedule/<id>
    // DAA: Using Sorting (ORDER BY day+slot), Hash Map (room/slot lookup)
    server.Get("/api/weekly_schedule/([0-9]+)", [](const Request& req, Response& res) {
        int tid = atoi(req.matches[0].c_str());
        sqlite3* db = openDB(); if (!db) { res.set_content(jerr("DB error"),"application/json"); return; }
        std::map<int,std::string> roomMap;  // Using Hash Map
        sqlite3_stmt* st = NULL;
        sqlite3_prepare_v2(db,"SELECT room_id, room_name FROM rooms",-1,&st,NULL);
        while(sqlite3_step(st)==SQLITE_ROW) roomMap[sqlite3_column_int(st,0)]=(const char*)sqlite3_column_text(st,1);
        sqlite3_finalize(st);
        // Using Sorting — ORDER BY day then slot
        const char* sql =
    ...
    // GET /free_rooms?day=MON&slot=3
    // DAA: Using Set (O(1) busy check)
    server.Get("/free_rooms", [](const Request& req, Response& res) {
        ...
        while(sqlite3_step(st)==SQLITE_ROW) allRooms[sqlite3_column_int(st,0)]=(const char*)sqlite3_column_text(st,1);
        sqlite3_finalize(st);
        std::set<int> busy;  // Using Set — O(1) lookup
    ...
    // POST /book_room
    // DAA: Stack (push undo), Linked List (room history), Hash Map (slot lookup)
    server.Post("/book_room", [](const Request& req, Response& res) {
        ...
        int sn=parseSlot(slotIn);  // Using Hash Map
        if(sn<0){res.status=400;res.set_content(jfail("Invalid slot"),"application/json");return;}
        std::string dayName=toUpper(date);
        sqlite3* db=openDB(); if(!db){res.set_content(jfail("DB error"),"application/json");return;}
        // Resolve room id
        int roomId=-1;
        ...
        // Using Stack (LIFO) — push for undo
        undoStack.push(Booking(tid,roomId,date,sn));
        // Using Linked List — room history
    ...
    // POST /undo_booking — Using Stack (LIFO)
    ...
    // GET /history/<room_id> — Using Linked List
    server.Get("/history/([0-9]+)", [](const Request& req, Response& res) {
        ...
        // Using Linked List — traverse and convert to list
