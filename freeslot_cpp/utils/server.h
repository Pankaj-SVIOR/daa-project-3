// =====================================================
// server.h  —  Simple HTTP Server (WinSock2)
// =====================================================
// Supports: GET, POST, cookies, redirects, query params
// Zero external dependencies — uses Windows sockets only
// =====================================================
...
// =====================================================
// Request — everything about an incoming HTTP request
// =====================================================
struct Request {
    std::string method;   // "GET" or "POST"
    std::string path;     // "/login"
    std::string body;     // POST body (JSON or form data)
    std::map<std::string, std::string> params;   // ?key=value
    std::map<std::string, std::string> cookies;  // Cookie: key=value
    std::vector<std::string> matches;            // URL capture groups
};

// =====================================================
// Response — what we send back to the browser
// =====================================================
struct Response {
    int         status;
    std::string content;
    std::string content_type;
    std::vector<std::string> extra_headers;  // for Set-Cookie etc.
    ...
    // Set a cookie (name, value, optional extra like "HttpOnly")
    ...
    // Clear a cookie (set it to expire in the past)
...
// =====================================================
// SimpleServer — main server class
// =====================================================
class SimpleServer {
public:
    ...
    // Start listening on given port (blocks forever)
...
private:
    ...
    // Parse raw HTTP into Request struct
    ...
    // Parse Cookie: header line -> map
    void parseCookies(const std::string& headers,
    ...
    // Match URL pattern with optional capture group ([0-9]+)
    bool matchPath(const std::string& pattern, const std::string& path,
        ...
        std::string suffix = (closeParen != std::string::npos)
                             ? pattern.substr(closeParen+1) : "";
...
// Helper: get cookie value safely
